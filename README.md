...
远程储存使用 Flarum 中转模式时，可将最近访问的对象缓存在本机
`storage/mie-files-cache`。缓存只保存文件内容；每次请求仍会先执行权限、
防盗链和下载统计。`local` 储存和公开直链不会重复使用本机缓存。缓存写入
失败时会自动回退到远程对象读取，不会影响文件可用性。

管理界面可设置：

- 是否启用本机缓存；
- 文件最后访问后的保留天数（默认 30 天）；
- 原文件缓存总容量（默认 2048 MB）；
- 单个原文件缓存上限（默认 256 MB）；
- 是否缓存缩略图及其容量上限（默认开启，可单独关闭）。

缓存会按最后访问时间执行 LRU 淘汰，并注册为每日计划任务。也可以手动运行：

```bash
php flarum mie-files:clean-cache
```

缓存是服务器上的明文私有副本，请像原始上传文件一样保护缓存目录和备份。
